...
class GlobalPlanner:
    ...
    def current_pose_callback(self, msg):
        ...
        # Calculate straight-line distance from vehicle to goal
        distance = np.hypot(
            self.current_location.x - self.goal_point.x,
            self.current_location.y - self.goal_point.y
        )

        # Check whether the vehicle is close enough to the goal
        if distance <= self.distance_to_goal_limit:
            # Publish an empty path so the path follower stops
            self.publish_lane_from_waypoints_list([])

            rospy.loginfo("goal position reached")

            # Clear the goal to prevent this from running repeatedly
            self.goal_point = None
    ...
    def convert_laneletseq_to_waypoints_list(self, laneletseq):
        ...
        for j, lanelet in enumerate(laneletseq):
            if "speed_ref" in lanelet.attributes:
                ...
                speed_kmh = min(float(lanelet.attributes['speed_ref']), self.speed_limit)
            else:
        ...
        if len(waypoints) >= 2 and self.goal_point is not None:
            ...
            for i in range(len(waypoints) - 1):
                ...
                # Project goal onto this segment
                ...
                # Save distance and segment index
